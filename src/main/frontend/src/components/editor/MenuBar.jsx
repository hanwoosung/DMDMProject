...
const MenuBar = ({editor, setFiles}) => {
        ...
        handleSaveFile(formData)
            .then((res) => {
                if (res.statusCode === 200) {
                    editor.chain().focus().setImage({src: res.data.filePath}).run();
    ...
    return (
        <div className={styles.menuBar}>
            {/* Undo */}
            <button data-tip="Undo" onClick={() => editor.chain().focus().undo().run()}
                    disabled={!editor.can().chain().focus().undo().run()}>
                <FaUndo />
            </button>

            {/* Redo */}
            <button data-tip="Redo" onClick={() => editor.chain().focus().redo().run()}
            ...
            {/* Bold */}
            <button data-tip="Bold" onClick={() => editor.chain().focus().toggleBold().run()}
                    className={editor.isActive("bold") ? styles.active : ""}>
                <FaBold />
            </button>

            {/* Italic */}
            <button data-tip="Italic" onClick={() => editor.chain().focus().toggleItalic().run()}
            ...
            {/* Alignments */}
            <button data-tip="Align Left" onClick={() => editor.chain().focus().setTextAlign("left").run()}
                    className={editor.isActive({textAlign: "left"}) ? styles.active : ""}>
                <FaAlignLeft />
            </button>
            <button data-tip="Align Center" onClick={() => editor.chain().focus().setTextAlign("center").run()}
                    className={editor.isActive({textAlign: "center"}) ? styles.active : ""}>
                <FaAlignCenter />
            </button>
            <button data-tip="Align Right" onClick={() => editor.chain().focus().setTextAlign("right").run()}
            ...
            {/* Add Code Block */}
            <button data-tip="Add Code Block" onClick={() => editor.chain().focus().toggleCodeBlock().run()}
